...
// ─── Adjacency map ────────────────────────────────────────────────────────────
// Which districts unlock when you complete a given district.
// "Complete" = visited 2 of 3 locations (enforced by backend; frontend just
// receives the unlock signal). Starting district is 101 (FiDi).
...
// ─── State ────────────────────────────────────────────────────────────────────
// In production this would be loaded from your Flask /progress/:user_id endpoint.
// For the hackathon demo, we use localStorage so state persists across refreshes.

const STORAGE_KEY = "manhattan_unlocked_state";
...
function loadState() {
    ...
    return {
        unlockedDistricts: ["101"],   // start at FiDi
        completedDistricts: [],       // districts fully done (2/3 visited)
        visitCounts: {}               // { "101": 1, "102": 2, ... }
    };
}
...
function getDistrictStyle(id) {
    ...
    if (state.unlockedDistricts.includes(sid)) {
        ...
        // Unlocked but not complete — lighter green/teal
...
function showSelectedPanel(id) {
    ...
    // Description — always shown when a district is selected
    ...
    // Body text (status/instructions — separate from description)
    ...
    // Upload section — only show if unlocked and not completed
...
// ─── Check-in logic ───────────────────────────────────────────────────────────
// This simulates the backend call. Replace simulateBackend() with a real
// fetch() to your Flask /checkin endpoint.

async function simulateBackend(districtId, file) {
    // REPLACE THIS with:
    // const form = new FormData()
    // form.append("photo", file)
    // form.append("user_id", YOUR_USER_ID)
    // form.append("location_id", districtId)
    // const res = await fetch("http://localhost:5000/checkin", { method: "POST", body: form })
    // return res.json()

    // Stub: always approves after a short delay, for demo purposes
    await new Promise(r => setTimeout(r, 1200));
    return { approved: true, message: "Check-in approved! (demo mode)" };
}
...
async function handleCheckIn() {
    ...
    const result = await simulateBackend(selectedDistrictId, file);
    ...
    if (state.visitCounts[sid] >= REQUIRED && !state.completedDistricts.includes(sid)) {
        ...
        saveState(state);
        // Re-render the selected panel to update pills
            ...
            onEachFeature: (feature, layer) => {
                ...
                layer.on({
                    mouseover(e) {
                        ...
                        // Show description preview in default panel (only when nothing is selected)
